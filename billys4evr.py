...
def fetch_history(
    ...
    # yfinance may return a multi-indexed column structure.  Flatten it so
    # pandas-ta can access the required OHLCV columns by name.
...
def add_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    # A few legacy indicators (e.g. mcgd) rely on deprecated pandas APIs.  Skip
    # those to prevent crashes while still computing the vast majority.
    df.ta.strategy("All", use_mp=False, ta_lib=False, exclude=["mcgd"])
    return df.tail(500)
